...
pub fn http_download<W: Write + Seek>(url: &String, writer: &mut W, timeout: core::time::Duration, connect_timeout: core::time::Duration, tls_backend: TlsBackend) -> Result<(), String>{
...
pub fn http_download_bytes(url: &String, timeout: core::time::Duration, connect_timeout: core::time::Duration, retries: usize, tls_backend: TlsBackend) -> Result<std::vec::Vec<u8>, String>{
	...
	loop {
		...
		match http_download(url, &mut buf, timeout, connect_timeout, tls_backend.clone()){
			Ok(_) => {
...
pub fn http_download_file(url: &String, file_path: &String, timeout: core::time::Duration, connect_timeout: core::time::Duration, retries: usize, tls_backend: TlsBackend) -> Result<(), String>{
	...
	loop {
		...
		match http_download(url, &mut file, timeout, connect_timeout, tls_backend.clone()){
			Ok(_) => {
				return Ok(());
			},
			Err(e) => {
